...
whole_text %>% 
  str_extract(thought_police)
```

Count how often Big Brother is mentioned.
Big Brother is mentioned 20 times.
```{r}
big_brother <- "[Bb][Ii][Gg] [Bb][Rr][Oo][Tt][Hh][Ee][Rr]"

whole_text %>% 
  str_extract_all(big_brother) %>% 
  flatten_chr() %>% 
  length()
```
...
No. Not does it refer to a she or a her.

It has 491 possible matches for names in the pattern capital letter, lowercase letter of any length, but it is possible these are just capitalised words like Thought Police, Big Brother or the start of sentences!

```{r}
julia <- "[Jj]ulia"

she <- " [she] [a-zA-Z:punct:]+"
...
```{r}
winston_date <- "[A-Z][a-z]+ [0-9]+th, [0-9]{4}"

whole_text %>% 
  str_extract_all(winston_date)

# Answers have
winston_date <- "[A-Z][a-z]+ [0-9]+(th|rd|st|nd), [0-9]{4}"

# Neil did
winston_date < "[A-Z][a-z]+ [0-9]+[a-z]+, [0-9]{4}"
...
```{r}
# Finding full name ----------------------------------------------------------
winston <- "Winston [A-Z][a-z]+"

whole_text %>% 
  str_extract_all(winston)

# Full name is Winston Smith

# Remove Name ----------------------------------------------------------------

winston_full <- "Winston Smith"

whole_text %>% 
  str_replace_all(c(winston, winston_full), "Character Name")
```
...
```{r}
# Find any instances where the gin mentioned is not Victory Gin ---------------
...
whole_text %>% 
  str_replace_all(capital_gin, capital_vodka) %>% 
  str_replace_all(lower_gin, lower_vodka)

# Homework has this because it's important not to replace gin in the middle of a word like beginning
...
```{r}
# Finding instances of departments -------------------------------------------
dept <- "[Dd][Ee][Pp][Aa][Rr][Tt][Mm][Ee][Nn][Tt]"

whole_text %>% 
  str_extract_all(dept)

# Only mentioned as "Department"

# Rebranding department -------------------------------------------------------
whole_text_rebrand <- whole_text %>% 
  str_replace_all("Department", "Section")
```


```{r}
# Finding instances of ministeries -------------------------------------------

ministries <- "[Mm][Ii][Nn][Ii][Ss][Tt][Rr][A-Za-z]+"

whole_text_rebrand %>% 
  str_extract_all(ministries) %>% 
  flatten_chr() %>% 
  unique()
# In the text Ministries are mentioned as "Ministry" and "Ministries

# Replacing Ministry with Department -------------------------------------------
whole_text_rebrand <- whole_text_rebrand %>% 
  str_replace_all(
    "Ministry", "Department"
  ) %>% 
  str_replace_all(
...
```{r}
# Find all Newspeak titles
newspeak_title <- "[Mm][Ii][Nn][Ii][a-zA-Z]+"

whole_text_rebrand %>% 
  str_extract_all(newspeak_title) %>% 
  flatten_chr() %>% 
  unique()

# We find Minitrue, Minipax, Miniluv and Miniplenty
```


```{r}
# Make a list of titles
titles <- whole_text_rebrand %>% 
  str_extract_all(newspeak_title) %>% 
  flatten_chr() %>% 
  unique()

# Replace "Mini" in newspeak titles with "Dep"

whole_text_rebrand %>%
  # find pattern newspeak_title
  str_extract_all(newspeak_title) %>% 
# Sub mini for dep
  str_replace_all("Mini", "Dep") 


#  if (item is in titles) {
#    str_replace("Mini", "Dep")
#  }
whole_text_rebrand %>% 
  str_replace_all(
    string = if_else(str_extract_all(newspeak_title) %in% titles,
    "Mini" == "Dep", 
    "Mini" == "Mini"))

gsub(pattern = "Mini", 
     x = whole_text_rebrand(newspeak_title), 
          replacement = "Dep")
...
# Do as I did with vodka
```

```{r}
# using a pattern to pull out a vector, 
newspeak_title <- "[Mm][Ii][Nn][Ii][a-zA-Z]+"

titles_to_replace <- whole_text_rebrand %>% 
  str_extract_all(newspeak_title) %>% 
  flatten_chr() %>% 
  unique()

# use str_sub to edit the vector 

new_titles <- titles_to_replace
  
str_sub(string = new_titles, start = 1L, end = 4L) <- "Dep"

# then replace anything in the first vector with its updated thing from the second vector.

test <- whole_text_rebrand %>% 
  str_replace_all(titles_to_replace[[1]], new_titles[[1]]) %>%
  str_replace_all(titles_to_replace[[2]], new_titles[[2]]) %>% 
  str_replace(titles_to_replace[[3]], new_titles[[3]]) %>% 
  str_replace(titles_to_replace[[4]], new_titles[[4]])
